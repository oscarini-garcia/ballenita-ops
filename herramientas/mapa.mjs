...
/**
 * Todas las migraciones de D1 juntas, no solo el esquema inicial.
 *
 * Leyendo solo `0001_esquema.sql` salía un desfase **falso** por cada tabla
 * nacida después —`planIdeas` está en la 0006 y `mejoras` en la 0010—, y un
 * aviso falso es peor que ninguno: enseña a ignorarlos.
 */
function migraciones() {
  const dir = join(RAIZ, 'api/migraciones');
...
function tablas() {
  ...
  // **Todas** las migraciones, no solo el esquema inicial. Leyendo solo
  // `0001_esquema.sql` daba un desfase falso por cada tabla nacida después
  // —`planIdeas` y `mejoras` viven en la 0006 y la 0010—, y un aviso falso es
